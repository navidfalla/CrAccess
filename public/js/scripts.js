	          ...
	          if(data.status == 'success') {
				  ...
				  $("td.fields").delegate("button.save-changes","click",function(){
					  ...
					  {"id":myid,"address":$("td#fields-"+myid+" input[name='address']").val(),"description":$("td#fields-"+myid+" textarea[name='description']").val(),"summary":$("td#fields-"+myid+" input[name='summary']").val()},
				  	  function(data) {
				  		if(data.status == 'success') {
				  			myhtml = "<b>Address</b>: <span>"+data.address+"</span> <br /><b>Summary</b>: <span>"+data.summary+"</span> <br /><b>Reporter</b>: <span>"+data.username+"</span> <br /><b>Date Added</b>: <span>"+data.date_added+"</span> <br /><a class=\"btn btn-default linkbutton\" href=" +baseURL+ "/issues/view/" + data.id + ">View</a><button class=\"btn btn-default report-solved\" id=\"solve-" +data.id+ "\">Report Solved</button><span class=\"note\"><em class=\"num-reports\">"+data.solved+"</em> reports solved</span><br/><button class=\"btn btn-default edit-issue\" id = \"edit-"+data.id+"\">Edit</button><button class = \"btn btn-default delete-issue\" id = \"delete-"+data.id+"\">Delete</button></td></tr>";
				  			$("td#fields-"+myid).html(myhtml);
